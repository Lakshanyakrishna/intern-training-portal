...
function extendMaterial(BaseMaterial: new (params?: Record<string, unknown>) => THREE.Material, cfg: ShaderConfig) {
...
const MergedPlanes = forwardRef<THREE.Mesh, MergedPlanesProps>(({ material, width, count, height }, ref) => {
  ...
  useFrame((_, delta) => {
    // Imperative r3f escape hatch: driving the shader clock every frame is the
    // intended way to animate a Three.js material outside React's render cycle.
    // eslint-disable-next-line react-hooks/immutability
    shaderMaterial.uniforms.time.value += 0.1 * delta;
  });
