    ...
    this.program = new Program(this.gl, {
      ...
      fragment: `
        ...
        void main() {
          ...
          vec3 finalColor = mix(color.rgb, vec3(1.0), 0.08 * uHighlight);
          float ring = smoothstep(-0.035, -0.028, d) - smoothstep(-0.006, 0.0, d);
          finalColor = mix(finalColor, vec3(1.0), ring * uHighlight);
...
class App {
  ...
  setHighlight(text: string | null) {
    this.medias.forEach(media => {
      media.program.uniforms.uHighlight.value = text && media.text === text ? 1 : 0;
    });
  }
...
export default function CircularGallery({
  ...
  useEffect(() => {
    ...
    resolveFont(font, fontUrl).then(resolvedFont => {
      ...
      appRef.current = app;
      app.setHighlight(highlightText);
    });
  ...
  // Highlighting doesn't rebuild the gallery or touch scroll position — it only updates a
  // per-card shader uniform, so switching the selected forte never jumps or re-scrolls it.
  useEffect(() => {
    appRef.current?.setHighlight(highlightText);
  }, [highlightText]);
  return (
    <div
