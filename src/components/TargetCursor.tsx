...
const TargetCursor = ({
  ...
  useEffect(() => {
    ...
    const enterHandler = (e: MouseEvent) => {
      ...
      activeTarget = target;
      const corners = Array.from(cornersRef.current);
      corners.forEach(corner => gsap.killTweensOf(corner, { x: true, y: true } as any));

      gsap.killTweensOf(cursorRef.current, { rotation: true } as any);
      ...
      const leaveHandler = () => {
        ...
        gsap.set(activeStrengthRef, { current: 0, overwrite: 'auto' });
        activeTarget = null;
        ...
        if (cornersRef.current) {
          const corners = Array.from(cornersRef.current);
          gsap.killTweensOf(corners, { x: true, y: true } as any);
