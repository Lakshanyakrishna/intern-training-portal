...
const statusConfig: Record<string, { label: string; color: string; bg: string }> = {
  ...
  'approved': { label: 'Approved', color: 'text-accent', bg: 'bg-neutral-50 dark:bg-neutral-900/20' },
};
  ...
  if (submissionFeedback.length === 0) {
    return (
      <div className="max-w-xl mx-auto text-center pt-16">
        ...
        <h1 className="text-2xl font-bold text-primary mb-2">Mentor Feedback</h1>
        ...
        <Link
          to="/"
          className="inline-flex items-center gap-2 px-5 py-2.5 bg-accent text-white font-medium rounded-lg hover:bg-accent-hover transition-colors text-sm"
        >
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-bold text-primary">Mentor Feedback</h1>
          <p className="text-sm text-secondary">Feedback on your submitted work</p>
        </div>
      </div>
      ...
      <div className="space-y-4">
        {[...submissionFeedback]
          ...
          return (
            <div key={fb.id} className="bg-surface border border-line rounded-xl p-5">
              <div className="flex items-start justify-between mb-4">
                <div>
                  <h3 className="text-sm font-semibold text-primary">{fb.submissionTitle}</h3>
                  <p className="text-xs text-secondary mt-0.5">{fb.moduleTitle}</p>
                </div>
              ...
              {fb.status !== 'waiting-review' && (
                <div className="space-y-4">
                  <div className="grid sm:grid-cols-2 gap-4">
                    <div>
                      <p className="text-xs font-semibold text-secondary uppercase tracking-wider mb-2">Strengths</p>
                    ...
                    <div>
                      <p className="text-xs font-semibold text-secondary uppercase tracking-wider mb-2">Needs Improvement</p>
                      <ul className="space-y-1">
                  ...
                  <div className="bg-surface-alt rounded-lg p-3">
                    <p className="text-xs font-semibold text-secondary uppercase tracking-wider mb-1">Overall Comment</p>
                    <p className="text-sm text-gray-700 dark:text-gray-300">{fb.overallComment}</p>
                  </div>

                  <div className="flex items-center gap-2">
                    <span className="text-xs font-semibold text-secondary uppercase tracking-wider">Mentor Score</span>
              ...
              {fb.status === 'waiting-review' && (
                <div className="bg-surface-alt rounded-lg p-3">
                  <div className="flex items-center gap-2">
                    ...
                    <p className="text-sm text-secondary">Awaiting mentor review</p>
                  </div>
                </div>
              )}
            </div>
