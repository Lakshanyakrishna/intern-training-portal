  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-secondary mt-1">Monitor notifications, email delivery, and templates.</p>
      </div>

      {/* Summary cards */}
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {statCard('Total Notifications', notifSummary.total, 'text-accent')}
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        {statCard('Emails Pending', emailSummary.pending, 'text-secondary')}
        {statCard('Total Emails', emailSummary.total, 'text-primary')}
      </div>
          ...
          {emailLogs.length === 0 ? (
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-line text-left text-xs text-secondary uppercase tracking-wider">
            ...
            templates.map(t => (
              <div key={t.id} className="bg-surface border border-line rounded-xl overflow-hidden">
                <div className="px-4 py-3 border-b border-line bg-surface-alt flex items-center justify-between">
                  <h3 className="text-sm font-semibold text-primary">{t.eventType}</h3>
                  ...
                  <div className="p-4 space-y-3">
                    <div>
                      ...
                      <input
                        ...
                        className="w-full text-sm border border-line rounded-lg px-3 py-2 bg-surface text-primary focus:outline-none focus:ring-2 focus:ring-accent"
                      />
                    </div>
                    <div>
                      <label className="block text-xs font-medium text-secondary mb-1">Email Body</label>
                      <textarea
                        rows={3}
                        value={editForm.emailBody}
                        onChange={e => setEditForm(prev => ({ ...prev, emailBody: e.target.value }))}
                        className="w-full text-sm border border-line rounded-lg px-3 py-2 bg-surface text-primary focus:outline-none focus:ring-2 focus:ring-accent resize-y font-mono"
                      />
                    </div>
                    <div>
                      <label className="block text-xs font-medium text-secondary mb-1">In-App Template</label>
                      <textarea
                        rows={3}
